...
DATA_DIRECTORY = '/Users/stan/Documents/uni/year_3/rp/seq2point-nilm-master/data/ukdale/'
SAVE_PATH = 'kettle/'
AGG_MEAN = 522
AGG_STD = 814
...
def get_arguments():
    ...
    parser.add_argument('--data_dir', type=str, default=DATA_DIRECTORY, help='The directory containing the UKDALE data')
    parser.add_argument('--appliance_name', type=str, default='kettle', help='which appliance you want to train: '
                                                                             'kettle, microwave,fridge,dishwasher,'
                                                                             'washingmachine')
...
def main():
    ...
    # Loop over all the houses for the specific appliance. In this case that is always house 1 and 2. params_appliances
    # is defined as a dict in ukdale_parameters.py
    for h in params_appliance[appliance_name]['houses']:
        # Prints the location of the directory where the data for the house is located.
        ...
        # Gets the aggregate reading for the house. Returns a dataframe with 2 columns. One for the 'time' and one for
        # the 'data'.
        mains_df = load_dataframe(args.data_dir, h, 1)

        # Links the house to the corresponding channel for that appliance. Returns a dataframe with 2 columns. One for
        # the 'time' and one for the 'aggregate'.
        ...
        mains_df['time'] = pd.to_datetime(mains_df['time'], unit='s')       # Converts to pandas datetime object
        mains_df.set_index('time', inplace=True)                            # Sets the time as the index for the array
        mains_df.columns = ['aggregate']                                    # Set the label of the column to 'aggregate'
        # resample = mains_df.resample(str(sample_seconds) + 'S').mean()
        mains_df.reset_index(inplace=True)                                  # Sets the index back to normal again
        ...
        # Appliance
        app_df['time'] = pd.to_datetime(app_df['time'], unit='s')           # Converts to pandas datetime object
    ...
    # Validation CSV
    val.to_csv(args.save_path + appliance_name + '_validation_' + '.csv', mode='a', index=False)

    # Training CSV
    train.to_csv(args.save_path + appliance_name + '_training_.csv', mode='a', index=False)
